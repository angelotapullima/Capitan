...
class UsuarioController{
    ...
    public function listar_usuario_por_id(){
        $id = $_POST['id'];
        $model = $this->usuario->obtener_por_id($id);
        $resources = array();
        $resources[0] = array(
            "usuario_id"=>$model->usuario_id,
            "nombre"=>$model->usuario_nombre,
            "dni"=>$model->usuario_dni,
            "nacimiento"=>$model->usuario_nacimiento,
            "sexo"=>$model->usuario_sexo,
            "email"=>$model->usuario_email,
            "telefono"=>$model->usuario_telefono,
            "usuario"=>$model->usuario_usuario,
            "posicion"=>$model->usuario_posicion,
            "habilidad"=>$model->usuario_habilidad,
            "num"=>$model->usuario_num,
            "foto"=>$model->usuario_foto
        );
        $data = array("results" => $resources);
        echo json_encode($data);
    }
    ...
    public function listar_chats_por_id_usuario(){
        ...
        for ($i=0;$i<count($model);$i++) {
            ...
            $datos_u = $this->usuario->obtener_por_id($model[$i]->usuario_1);
            $datos_ = $this->usuario->obtener_por_id($model[$i]->usuario_2);
