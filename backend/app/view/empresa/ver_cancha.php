        ...
        <?php
            if($cancha->usuario_id==$usuario_id){
                        ...
                        if(count($resumen_hoy)>0){
                            ...
                            foreach($resumen_hoy as $r){
                                if($r->reserva_hora == $hora){
                                    ...
                                    $monto_ = $r->reserva_pago1 + $r->reserva_pago2;
                                    ?>
                                    <button type="button" onclick="form_reserva1('<?= $ff ?>','<?= $hora ?>','<?= $precioD ?>','<?= $precioN ?>','<?= $nombre_; ?>','<?= $monto_; ?>','<?= $estado; ?>','<?= $reserva_id ?>')"  data-toggle="modal" data-target="#exampleModal1" class="btn btn-sm <?= $color; ?>" style="margin-bottom: 5px; font-size: 9pt;font-weight: bold; padding-bottom: 0; color: #fff">
                                        <?= $hora; ?>
                                    </button>
                                    <?php
                                    $encontro = 1;
                        ...
                        if(count($resumen_m)>0){
                            ...
                            foreach($resumen_m as $r){
                                if($r->reserva_hora == $hora){
                                    ...
                                    $monto_ = $r->reserva_pago1 + $r->reserva_pago2;
                                    ?>
                                    <button type="button" onclick="form_reserva1('<?= $ff2 ?>','<?= $hora ?>','<?= $precioD ?>','<?= $precioN ?>','<?= $nombre_; ?>','<?= $monto_; ?>','<?= $estado; ?>','<?= $reserva_id ?>')"  data-toggle="modal" data-target="#exampleModal1" class="btn btn-sm <?= $color; ?>" style="margin-bottom: 5px; font-size: 9pt;font-weight: bold; padding-bottom: 0; color: #fff">
                                        <?= $hora; ?>
                                    </button>
                                    <?php
                                    $encontro2 = 1;
                        ...
                        if(count($resumen_p)>0){
                            ...
                            foreach($resumen_p as $r){
                                if($r->reserva_hora == $hora){
                                    ...
                                    $monto_ = $r->reserva_pago1 + $r->reserva_pago2;
                                    ?>
                                    <button type="button" onclick="form_reserva1('<?= $ff3 ?>','<?= $hora ?>','<?= $precioD ?>','<?= $precioN ?>','<?= $nombre_; ?>','<?= $monto_; ?>','<?= $estado; ?>','<?= $reserva_id ?>')"  data-toggle="modal" data-target="#exampleModal1" class="btn btn-sm <?= $color; ?>" style="margin-bottom: 5px; font-size: 9pt;font-weight: bold; padding-bottom: 0; color: #fff">
                                        <?= $hora; ?>
                                    </button>
                                    <?php
                                    $encontro3 = 1;
                ...
                <div class="col-xs-6 col-md-2 col-lg-2" style="border-right: 1px solid grey">
                    ...
                    $horario_final = explode('-',$horario_[1]);
                    $hora_actual= date('H');
                    if($hora_actual>$horario_[0]){
                        $horario_[0] = $hora_actual;
                    }
                    for ($i = $horario_[0];$i<$horario_final[1];$i++){
                        $j = $i + 1;
                        $hora = $i . ":00 - " . $j .":00";
                        if(count($resumen_hoy)>0){
                            ...
                            foreach($resumen_hoy as $r){
                                if($r->reserva_hora == $hora){
                                    ...
                                    ?>
                                    <button type="button" class="btn btn-sm btn-danger" style="margin-bottom: 5px; font-size: 9pt;font-weight: bold; padding-bottom: 0; color: #fff"><?= $hora; ?></button>
                                    <?php
                                    $encontro = 1;
                                }
                            }
                            if($encontro==0){
                                ...
                                <form method="post" action="<?= _SERVER_ ?>Empresa/reservar_cancha">
                                    ...
                                    <input type="hidden" name="cancha" value="<?= $id; ?>">
                                    <input type="submit" style="margin-bottom: 5px; font-size: 9pt;font-weight: bold; padding-bottom: 0; color: #fff" class="btn btn-sm btn-success" value="<?= $hora; ?>">
                                </form>
                                <?php
                            }
                        }else{
                    ...
                    for ($i = $horario2_[0];$i<$horario_final2[1];$i++){
                        ...
                        if(count($resumen_m)>0){
                            ...
                            if($encontro2==0){
                                ?>
                                <form method="post" action="<?= _SERVER_ ?>Empresa/reservar_cancha">
                                    <input type="hidden" name="fecha" value="<?= $manhana; ?>">
                                    <input type="hidden" name="hora" value="<?= $hora;?>">
                                    <input type="hidden" name="cancha" value="<?= $id; ?>">
                                    <input type="submit" style="margin-bottom: 5px; font-size: 9pt;font-weight: bold; padding-bottom: 0; color: #fff" class="btn btn-sm btn-success" value="<?= $hora; ?>">
                                </form>
                    ...
                    for ($i = $horario3_[0];$i<$horario_final3[1];$i++){
                        ...
                        if(count($resumen_p)>0){
                            ...
                            if($encontro3 == 0){
                                ?>
                                <form method="post" action="<?= _SERVER_ ?>Empresa/reservar_cancha">
                                    <input type="hidden" name="fecha" value="<?= $pasado; ?>">
                                    <input type="hidden" name="hora" value="<?= $hora;?>">
                                    <input type="hidden" name="cancha" value="<?= $id; ?>">
                                    <input type="submit" style="margin-bottom: 5px; font-size: 9pt;font-weight: bold; padding-bottom: 0; color: #fff" class="btn btn-sm btn-success" value="<?= $hora; ?>">
                                </form>
