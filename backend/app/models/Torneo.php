...
class Torneo{
    ...
    public function registrar_torneo($usuario_id,$nombre,$descripcion,$fecha,$hora,$lugar){
        $result = 2;
        try {
            ...
            $stm = $this->pdo->prepare($sql);
            $stm->execute([
                $usuario_id,$nombre,$descripcion,$fecha,$hora,$lugar
            ]);
            $result = 1;
        } catch (Exception $e){
            //throw new Exception($e->getMessage());
    ...
    public function listar_reto_por_id($reto_id){
        $result = [];
        try {
            $stm = $this->pdo->prepare("select * from reto where reto_id = ?");
            $stm->execute([$reto_id]);
            $result = $stm->fetch();
        } catch (Exception $e){
        }
